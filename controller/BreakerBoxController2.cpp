...
class BreakerBoxController2 : public SimpleController
{
    Link* breaker;
...
public:
    virtual bool initialize(SimpleControllerIO* io) override
    {
        ...
        breaker = ioBody->link(prefix + "LEVER_L");
        lights = ioBody->devices();

        if(!breaker) {
            (*os) << prefix << "LEVER_L is not found." << std::endl;
        }
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(BreakerBoxController2)
