#include "FireController1.h"
#include <cnoid/EigenUtil>

using namespace std;
using namespace cnoid;

static FireController1* instance = nullptr;


FireController1* FireController1::instance()
{
    return ::instance;
}


FireController1::FireController1()
{
    ::instance = this;
}


bool FireController1::initialize(SimpleControllerIO* io)
{
    this->io = io;
...
void FireController1::notifyWaterFlowTarget(const Vector3& position, double strength)
{
    std::lock_guard<std::mutex> lock(waterMutex);
...
void FireController1::updateFire()
{
...
void FireController1::initializeTestMode()
{
...
void FireController1::controlTestMode()
{
    if(doUpdateFire){
        auto& ps = fire->particleSystem();
        if(testMode == 1){
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(FireController1)
