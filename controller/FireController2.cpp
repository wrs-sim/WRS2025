#include "FireController2.h"
#include <cnoid/EigenUtil>

using namespace std;
using namespace cnoid;

static FireController2* instance = nullptr;


FireController2* FireController2::instance()
{
    return ::instance;
}


FireController2::FireController2()
{
    ::instance = this;
}


bool FireController2::initialize(SimpleControllerIO* io)
{
    this->io = io;
...
void FireController2::notifyWaterFlowTarget(const Vector3& position, double strength)
{
    std::lock_guard<std::mutex> lock(waterMutex);
...
void FireController2::updateFire()
{
...
void FireController2::initializeTestMode()
{
...
void FireController2::controlTestMode()
{
    if(doUpdateFire){
        auto& ps = fire->particleSystem();
        if(testMode == 1){
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(FireController2)
