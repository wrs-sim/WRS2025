...
class HoseNozzleController2 : public SimpleController
{
    ...
    MarkerDevice* marker;
    FireController2* fireController;
    int mode;
...
public:
    ...
    virtual bool initialize(SimpleControllerIO* io) override
    {
        ...
        if(!nozzle || !valve || !hoseConnector || /* nodePath.empty() || */ !lever ||
           !reactionJoint1 || !reactionJoint2 || !water || !marker){
            io->os() << "HoseNozzleController2: Eequired body components are not completely detected." << endl;
            return false;
        }
        ...
        if(mode > 0){
            io->os() << "HoseNozzleController2: test mode " << mode << endl;
        }
        if(mode == 1){
    ...
    {
        fireController = FireController2::instance();
        return true;
    }
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(HoseNozzleController2)
